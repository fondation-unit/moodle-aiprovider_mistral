...
use aiprovider_mistral\helper;
use core_ai\form\action_settings_form;

/**
 * Base action settings form for Mistral provider.
 *
...
class action_form extends action_settings_form {
    ...
    protected function definition(): void {
        ...
        $this->providername = $this->_customdata['providername'] ?? 'aiprovider_mistral';
    ...
    public function validation($data, $files): array {
        ...
        if (!empty($data['modelextraparams'])) {
            ...
            if (json_last_error() !== JSON_ERROR_NONE) {
                $errors['modelextraparams'] = get_string('invalidjson', 'aiprovider_mistral');
            }
        }
    ...
    protected function add_model_fields(int $modeltype): void {
        ...
        $PAGE->requires->js_call_amd('aiprovider_mistral/modelchooser', 'init');
        $mform = $this->_form;
        ...
        if (
            ...
            $defaultmodel = ($actionname === 'generate_image') ? AIPROVIDER_MISTRAL_MEDIUM_MODEL : AIPROVIDER_MISTRAL_SMALL_MODEL;
        } else {
        ...
        $mform->addElement(
            'select',
            'modeltemplate',
            get_string("action:{$this->actionname}:model", 'aiprovider_mistral'),
        ...
        $mform->addHelpButton('modeltemplate', "action:{$this->actionname}:model", 'aiprovider_mistral');

        $mform->addElement('hidden', 'model', $defaultmodel);
        $mform->setType('model', PARAM_TEXT);

        $mform->addElement('text', 'custommodel', get_string('custom_model_name', 'aiprovider_mistral'));
    ...
    /**
     * Get the list of models from Mistral API.
     *
     * @param int $modeltype Model type.
    ...
    protected function get_model_list(int $modeltype): array {
        $models = [];
        $apikey = $this->_customdata['providerconfig']['apikey'] ?? '';

        if (!empty($apikey)) {
            $apimodels = \aiprovider_mistral\helper::get_available_models($apikey);

            // Filter by type if needed.
            if ($modeltype === \aiprovider_mistral\aimodel\mistral_base::MODEL_TYPE_IMAGE) {
                // Only models that support image generation.
                $apimodels = array_filter(
                    $apimodels,
                    fn($id) => str_contains($id, 'medium') || str_contains($id, 'large'),
                    ARRAY_FILTER_USE_KEY
                );
            }

            foreach ($apimodels as $id => $label) {
                $models[$id] = $label;
            }
        }

        // Add static fallbacks in case the API call fails.
        $staticmodels = [
            'mistral-small-latest'  => 'Mistral Small (latest)',
            'mistral-medium-latest' => 'Mistral Medium (latest)',
            'mistral-large-latest'  => 'Mistral Large (latest)',
        ];
        foreach ($staticmodels as $id => $label) {
            if (!isset($models[$id])) {
                $models[$id] = $label;
            }
        }

        // Custom model option always last.
